...
const baseColors: Record<Environment, string> = {
  ...
  "PROD": "border-emerald-500/40 text-emerald-700 dark:text-emerald-400",
};
...
const activeColors: Record<Environment, string> = {
  ...
  "PROD": "bg-emerald-500/15 border-emerald-500/60 text-emerald-700 dark:text-emerald-300",
};

export function EnvironmentPill({ env, active = false, done = false }: EnvironmentPillProps) {
  const colorClass = active ? activeColors[env] : done ? "border-slate-400 text-slate-500 line-through dark:border-slate-600" : `border-slate-300 dark:border-slate-700 ${baseColors[env]}`;
  return (
