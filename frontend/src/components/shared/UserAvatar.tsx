...
export function UserAvatar({ userId, size = "md", showName = false, tooltipAlign = "center" }: UserAvatarProps) {
  ...
  return (
    <span className="group/avatar relative inline-flex items-center gap-2" aria-label={tooltip}>
      ...
      <span className={`pointer-events-none absolute top-0 z-50 -translate-y-[calc(100%+8px)] whitespace-nowrap rounded-md border border-border bg-popover px-2 py-1 text-xs font-medium text-popover-foreground opacity-0 shadow-md transition-opacity duration-150 group-hover/avatar:opacity-100 group-focus-within/avatar:opacity-100 ${tooltipAlignClass}`}>
        {tooltip}
      </span>
